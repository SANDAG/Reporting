    ...
    public HashMap<String, Double> getBoardingsByMainMode(Short scenarioId)
    {
        ...
        for (Object[] record : list)
        {
            ...
            switch (((Byte)record[1]).byteValue()) {
                case COMMUTER_RAIL:
                    mainModeBoardings.put(COMMUTER_RAIL_KEY, new Double(((BigDecimal)record[0]).doubleValue()));
                    break;
                case LIGHT_RAIL:
                    mainModeBoardings.put(LIGHT_RAIL_KEY, new Double(((BigDecimal)record[0]).doubleValue()));
                    break;
                case REGIONAL_BRT:
                    mainModeBoardings.put(REGIONAL_BRT_KEY, new Double(((BigDecimal)record[0]).doubleValue()));
                    break;
                case EXPRESS_BUS:
                    mainModeBoardings.put(EXPRESS_BUS_KEY, new Double(((BigDecimal)record[0]).doubleValue()));
                    break;
                case LOCAL_BUS:
                    mainModeBoardings.put(LOCAL_BUS_KEY, new Double(((BigDecimal)record[0]).doubleValue()));
                    break;
            }
        }
